...
Function getHTMLContent(NO, txtContent)
    ...
    Next
        ...
        regEx.Pattern = "</?table[\s\S]*?>|</?td[\s\S]*?>|</?tr[\s\S]*?>|</?div[\s\S]*?>|</?textarea[\s\S]*?>"
        s = regEx.Replace(s, "" )
    getHTMLContent = s
    'getHTMLContent = delhtml(s)
End Function
...
Function SafeRequest(ParaValue) 
	...
	re.IgnoreCase=False
	re.Global=True
	re.Pattern="'|Select|Update|Delete|insert|Count|droptable|truncate|Asc|Mid|char|xp_cmdshell|execmaster|netlocalgroupadministrators|And|netuser|Or" '需要过滤得字符以“|”隔开
