...
<table cellpadding="0" cellspacing="0" align="center" class="Admin_Table">
  $Admin_ShowHTML$
  <tr>
    <td colspan="2" align="center"><input type="button"  value="修改" onclick="window.location.href='<%=NowFile%>?action=modi&$AutoFieldName$=<%=$AutoFieldName$%>&ModiURL=<%=ModiURL%>'" />
...
<table cellpadding="0" cellspacing="0"  align="center"  class="Admin_Table" id="trColor">
  <tr>
    <th>编号</th>
    $Admin_ListHTML1$
    <th>操作<input type="checkbox" onclick="CheckBox('trColor')" title="反选"><a href="javascript:DelChkID('<%=NowFile%>?action=delok&$AutoFieldName$=','trColor')" title="删除所有选中记录">删</a></th>
  </tr>
<%
'===========================分页设置===========================
Set Page1 = New Page
...
%>
  <tr>
    ...
    $Admin_ListHTML2$
    <td><a href="<%=NowFile%>?action=modi&$AutoFieldName$=<%=rs("$AutoFieldName$")%>">修改</a> <a href="<%=NowFile%>?action=delok&$AutoFieldName$=<%=rs("$AutoFieldName$")%>" onclick="return confirm('确定删除？')">删除</a> <input type="checkbox" value="<%=rs("$AutoFieldName$")%>"></td>
  </tr>
<%
rs.movenext
Next
