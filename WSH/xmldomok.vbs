...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''' 
    ...
    set objxml=createObject("microsoft.xmldom") '创建 XMLDOM 对象 
        ...
        if objxml.parseError<>0 then '//检测文件是否存在 0为存在;不等于0为不存在 
            ...
            objxml.selectSingleNode("//root/Child1").appendChild(objxml.createElement("Children1")).text="<h1>Child1 Children1 text</h1>" 
             
            '-'' //创建子节点1 Child1 的子节点 Children2, 并添加 CDATA 文本 
            objxml.selectSingleNode("//root/Child1").appendChild(objxml.createElement("Children2")) 
            objxml.selectSingleNode("//root/Child1/Children2").appendChild(objxml.createCDATASection("<h2>Child1 Children2 CDATASection</h2>")) 
            ...
            xmlstrs=xmlstrs &  objxml.selectSingleNode("//root").text&"<br/>" 
             
            '''-'' // 读出子节点 Child1 及其子节点的所有文本数据 
            xmlstrs=xmlstrs &  objxml.selectSingleNode("//root/Child1").text&"<br/>" 
             
            ''''-' // 读出子节点 Child1 的子节点 Children2 的文本数据 
            xmlstrs=xmlstrs &  objxml.selectSingleNode("//root/Child1/Children2").text&"<br/>" 
             
            ''''-'' // 为 节点1 的子节点 Children3 赋文本值, 并读出文本数据 
            objxml.selectSingleNode("//root/Child1/Children3").text="Root Child1 Children3 text" 
            xmlstrs=xmlstrs &  objxml.selectSingleNode("//root/Child1/Children3").text&"<br/>" 
    ...
    '如果只使用 xmlstrs=xmlstrs &  读出 XML 文件数据, 大概不需要使用 CDATASECTION, xmlstrs=xmlstrs &  自动 转换 
